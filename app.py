...
# 완벽한 밀착 및 잔상 방지를 위한 초강력 CSS 스타일링
st.markdown(
    """
    <style>
    /* 전체 페이지 여백 제거 */
    ...
    /* 사과 버튼: 배경/테두리 완전 투명화하여 사과와 숫자만 보이게 밀착 */
    div.stButton > button {
...
if st.session_state.game_state == "ready":
    ...
    with col_m:
        ...
        if st.button("🚀 Start 게임 시작", key="pure_lobby_start_btn", use_container_width=True):
            start_game()
            st.rerun()
